...
- Декомпиляция с импортом `Lang.dat` теперь сравнивает числовые `Script/<имя>/<номер>`-ключи исходного и пересобранного SCR. При перенумерации выдаётся `rscript-dialog-language-key-renumbered`, RSON не публикуется, а `script compare-scr` показывает изменение ключей отдельно.
