...
- Исправлены ложные ошибки для графовых `TVar Array`: повторный `newarray` не обязателен, а доказанный `ArrayDelete(array, 0)` допускает последующий обход с нуля до сброса массива.
- Прямой ограниченный обход `GalaxyStar(i)` при `0 <= i < GalaxyStars()` распознаётся без лишнего null-guard; сохранённые индексы, изменение счётчика и условные присваивания не получают общего исключения.
- `script build` и Python API принимают точечный `--allow CODE[:GLOB]`. Project workflow передаёт свои исключения в RSON-компиляцию; подавленные сообщения остаются в JSON, а набор исключений входит в ключ кэша.
- Исправлен кэш исходников внутри папки мода: случайный путь staging больше не вызывает постоянную пересборку; `project plan` и build согласованы.
- Добавлены перекрёстные тесты CacheData для BlockParEditor 1.9/2.1 и трёх кодировок, а также реальная MSVC x86-сборка Native Loader-плагина с проверкой отсутствующей DLL, кэша и повторяемости ZIP.
- Обновлены руководство по скриптам, документация проектной сборки и скилл для нейросетей.

Проверка выпуска: 400 тестов и 42 подтеста прошли, 2 теста пропущены.
Четыре интеграционных теста останавливаются при legacy `SVR → RSON` с
`Runtime error 217` внешнего RScript 4.10f. Сбой воспроизведён и до этих
изменений; он не считается исправленным. RScript 4.15f отдельно успешно
пересобрал оба проверяемых планетарных мода. Не отключайте проверку результата
ради обхода ошибки старого EXE.
