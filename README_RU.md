...
- Ограничение числа `TGroup` теперь зависит от обнаруженного компилятора: для RScript 4.10f/legacy больше четырёх групп блокируется, для 4.15f поддерживается современный лимит, а при неизвестной версии выдаётся предупреждение вместо ложной ошибки.
- Добавлено обнаружение RScript-функций, зарегистрированных через XenoNativeLoader: встроенные имена legacy-DLL больше не считаются неизвестными API, а `*.XenoScriptApi.json` позволяет проверить арность вызова и регистрацию функции.
- Аудит Native Loader теперь различает подтверждённую регистрацию, найденное в DLL имя без доказанной сигнатуры и отключённый плагин; отчёт не скрывает неполное покрытие.
- Обновлены инструкции структуры плагина и диагностика зависимостей, чтобы `ImportedFunction` можно было проверять без запуска DLL.
- Декомпиляция с импортом `Lang.dat` теперь сравнивает числовые ключи `Script/<имя>/<номер>` исходного и пересобранного SCR. При перенумерации выдаётся `rscript-dialog-language-key-renumbered`, RSON не публикуется, а `script compare-scr` показывает изменение ключей отдельно.

Результат полной проверки: 409 passed, 42 subtests passed, 2 skipped.
